...
class PolypDataset_aux(data.Dataset):
    ...
    def __init__(self, image_root, gt_root, aux_root, trainsize, augmentations):
        ...
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.png')]
        self.aux = [aux_root + f for f in os.listdir(aux_root) if f.endswith('.png')]
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
...
def get_loader_with_aux(image_root, gt_root, aux_root, batchsize, trainsize, shuffle=True, num_workers=4, pin_memory=True, augmentation=False):
